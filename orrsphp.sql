...
BEFORE INSERT ON orrs_train_tickets
FOR EACH ROW
BEGIN
    DECLARE next_ticket_id INT;
    SELECT IFNULL(MAX(ticket_id), 0) + 1 INTO next_ticket_id FROM orrs_train_tickets;
    SET NEW.ticket_id = next_ticket_id;
END;

CREATE TRIGGER generate_pass_id 
BEFORE INSERT ON orrs_passenger
FOR EACH ROW
BEGIN
    DECLARE next_pass_id INT;
    SELECT IFNULL(MAX(pass_id), 0) + 1 INTO next_pass_id FROM orrs_passenger;
    SET NEW.pass_id = next_pass_id;
END;
